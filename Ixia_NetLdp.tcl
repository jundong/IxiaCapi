...
Deputs "ldp:$ldp"
		...
		if { $intfhandle != "null"} {
		    ...
		    set rb_interface $intfhandle
			set hInt [ ixNet add $handle interface ]
			ixNet setM $hInt -protocolInterface $intfhandle -enabled True
			
			ixNet commit
			set hInt [ ixNet remapIds $hInt ]

			set interface($intfhandle) $hInt
            if { [ixNet getL $intfhandle ipv4] == "" } {
                ixNet add $intfhandle ipv4 
            }				
		} else {
...
# -- unsupported yet
	...
	if { [ info exists active ] } {
		ixNet setA $handle -enabled $active
		ixNet commit
	}
	...
	if { [ info exists ipv4_prefix_len ] } {
		foreach int $rb_interface {
			...
			ixNet setA $int/ipv4 -maskWidth  $ipv4_prefix_len
			ixNet commit
			}
		}
	}
	if { [ info exists ipv4_gw ] } {
		foreach int $rb_interface {
				...
				ixNet setA $int/ipv4 -gateway  $ipv4_gw
				ixNet commit
			}
			if { [ixNet getL $interface($int) "targetPeer"] == "" } {
			    set tpHandle [ixNet add $interface($int) "targetPeer"]
			    ixNet setMultiAttribute $tpHandle \
					-enabled true \
					-ipAddress $ipv4_gw
				ixNet commit
			}
