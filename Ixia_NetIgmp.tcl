...
Deputs "----- TAG: $tag -----"
		...
		ixNet commit
		if { $intfhandle != "null"} {
			ixNet setA $intfhandle -enabled true
			ixNet commit
		}
...
Deputs "----- TAG: $tag -----"
	...
	method start {} {
	    set tag "body IgmpHost::start [info script]"
Deputs "----- TAG: $tag -----"
        ixNet exec start $hPort/protocols/igmp
		after 2000
	}
	method stop {} {
	    set tag "body IgmpHost::stop [info script]"
Deputs "----- TAG: $tag -----"
...
Deputs "Args:$args "
    foreach { key value } $args {
        ...
        switch -exact -- $key {
            ...
            -version {
            	set version [string tolower $value]
            }
            -ipaddr {
            	set ipaddr $value
            }
...
Deputs "----- TAG: $tag -----"
	...
	method start {} {
		set tag "body MldHost::start [info script]"
	Deputs "----- TAG: $tag -----"
	...
	method stop {} {
		set tag "body MldHost::stop [info script]"
	Deputs "----- TAG: $tag -----"
