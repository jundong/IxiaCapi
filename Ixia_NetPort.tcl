...
#		8. Add IxHost class to emulate a host
# Version 1.7.2.6
...
# Version 1.21.4.27
#       27. modify IxHost::config  src_mac format
# Version 1.22.4.28
...
# Version 2.10.4.34
#		37. add ipv4_gw_step/ipv6_gw_step in IxHost.config
# Version 2.11.4.44
...
body Port::get_port_stats {} {

    set tag "body Port::get_port_stats [info script]"
    Deputs "----- TAG: $tag -----"
    ...
    set tx_frame_rate         	[ lsearch -exact $captionList {Frames Tx. Rate} ]
    set rx_frame_rate         	[ lsearch -exact $captionList {Valid Frames Rx. Rate} ]
    set tx_bytes                 [ lsearch -exact $captionList {Bytes Tx.} ]
    set rx_bytes                 [ lsearch -exact $captionList {Bytes Rx.} ]
    set tx_bits                 [ lsearch -exact $captionList {Bits Sent} ]
    set rx_bits                 [ lsearch -exact $captionList {Bits Received} ]
    set tx_bit_rate         	[ lsearch -exact $captionList {Tx. Rate (bps)} ]
    set rx_bit_rate       		[ lsearch -exact $captionList {Rx. Rate (bps)} ]
    set tx_byte_rate            [ lsearch -exact $captionList {Bytes Tx. Rate} ]
    set rx_byte_rate            [ lsearch -exact $captionList {Bytes Rx. Rate} ]
    set fcs_error_frame        	[ lsearch -exact $captionList {CRC Errors} ]
    set rx_data_integrity	    [ lsearch -exact $captionList {Data Integrity Frames Rx.} ]
    ...
    foreach row $stats {
        ...
        set statsItem   "tx_bits"
        set statsVal    [ lindex $row $tx_bits ]
        Deputs "stats val:$statsVal"
        set ret $ret[ GetStandardReturnBody $statsItem $statsVal ]
          
        set statsItem   "rx_bits"
        set statsVal    [ lindex $row $rx_bits ]
        Deputs "stats val:$statsVal"
        set ret $ret[ GetStandardReturnBody $statsItem $statsVal ]
        
        set statsItem   "tx_bytes"
        set statsVal    [ lindex $row $tx_bytes ]
        Deputs "stats val:$statsVal"
        set ret $ret[ GetStandardReturnBody $statsItem $statsVal ]
          
        set statsItem   "rx_bytes"
        set statsVal    [ lindex $row $rx_bytes ]
        Deputs "stats val:$statsVal"
        set ret $ret[ GetStandardReturnBody $statsItem $statsVal ]
        
        set statsItem   "tx_byte_rate"
        set statsVal    [ lindex $row $tx_byte_rate ]
        Deputs "stats val:$statsVal"
        set ret $ret[ GetStandardReturnBody $statsItem $statsVal ]
          
        set statsItem   "rx_byte_rate"
        set statsVal    [ lindex $row $rx_byte_rate ]
        Deputs "stats val:$statsVal"
        set ret $ret[ GetStandardReturnBody $statsItem $statsVal ]  
    }
...
class IxHost {
	...
	method config { args } {}
	method unconfig {} {
    set tag "body IxHost::unconfig [info script]"
		Deputs "----- TAG: $tag -----"
		set hPort ""
		chain
	}
...
body IxHost::config { args } {
    global errNumber
    
    set tag "body IxHost::config [info script]"
Deputs "----- TAG: $tag -----"
...
body IxHost::ping { args } {
    set tag "body IxHost::ping [info script]"
Deputs "----- TAG: $tag -----"
