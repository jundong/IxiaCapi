...
class SimulatedLink {
    ...
    constructor { router } {
		...
		set tag "body SimulatedLink::ctor [info script]"
Deputs "----- TAG: $tag -----"
		...
		if { [ catch {
			...
			error "$errNumber(1) Router Object in SimulatedLink ctor"
		}
		...
		set handle [ ixNet remapIds $hRouteRange ]
		Deputs "handle: $handle"
		ixNet setM $handle -enalbed True \
		     -origin sameArea
		ixNet commit
...
Deputs "Args:$args "
	...
	if { [ info exists origin ] } {
		ixNet setA $handle -origin $origin
	}
...
class SimulatedExternalRoute {
    ...
    constructor { router } {
		...
		set tag "body SimulatedExternalRoute::ctor [info script]"
Deputs "----- TAG: $tag -----"
		...
		if { [ catch {
			...
			error "$errNumber(1) Router Object in SimulatedExternalRoute ctor"
		}
