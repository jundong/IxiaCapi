...
body DhcpHost::get_per_session { args } {
    set tag "body DhcpHost::get_per_session [info script]"
Deputs "----- TAG: $tag -----"
    foreach { key value } $args {
        set key [string tolower $key]
        switch -exact -- $key {
    ...
    # set view  [ ixNet getF $root/statistics view -caption "Port Statistics" ]
	#set view {::ixNet::OBJ-/statistics/view:"DHCPv4 Per Session"}
Deputs "view:$view"
    set captionList             [ ixNet getA $view/page -columnCaptions ]
...
Deputs "caption list:$captionList"
	...
	set captionList         [ ixNet getA $view/page -columnCaptions ]
    set portIndex          [ lsearch -exact $captionList {Port Name} ]
    set ipIndex             [ lsearch -exact $captionList {IP Address} ]
    set gwIndex             [ lsearch -exact $captionList {Gateway Address} ]
...
# Deputs "chas:$chassis card:$card port$port"
    ...
    foreach row $stats {
        ...
        eval {set row} $row
Deputs "row:$row"
Deputs "portname:[ lindex $row $portIndex ]"
		# if { [ string length $card ] == 1 } {
			# set card "0$card"
		# }
		# if { [ string length $port ] == 1 } {
			# set port "0$port"
		# }
		if { [ixNet getA $hPort -name] != [ lindex $row $portIndex ] } {
			continue
		}
...
Deputs "new dot1x endpoint"
			...
			#-- add dhcp endpoint stack
			set sg_dot1xEndpoint [ixNet getL $sg_ethernet dot1x]
			if {$sg_dot1xEndpoint==""} {
...
Deputs "Args:$args "
    foreach { key value } $args {
        ...
        switch -exact -- $key {
			...
			-auth_period {
				set auth_period  [expr $value / 1000]
            }
			-start_period {
				set start_period [expr $value / 1000]
			}
			-max_start {
				set max_start $value
			}
			-mac_addr {
				set mac_addr [ IxiaCapi::Regexer::MacTrans $value ]
			}
			-enabled {
			    set value [ string tolower $value ]
				if { $value == "enable" } {
	...
	if { [ info exists mac_addr ] } {
		ixNet setA $handle/macRange -mac $mac_addr
        ixNet commit
	}
	if { [ info exists count ] } {
	...
	if {  [ info exists ca_path ] || [ info exists key_path ] || [ info exists auth_period ]  \
	       || [ info exists start_period ] || [ info exists max_start ] } {
	    set root [ixNet getRoot]
		...
		ixNet commit
		if { [info exists auth_period] } {
			ixNet setA $sg_dot1xGlobal \
				-authPeriod $auth_period
		} 
		if { [info exists start_period] } {
			ixNet setA $sg_dot1xGlobal \
