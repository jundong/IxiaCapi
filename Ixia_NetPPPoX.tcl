...
body PppoeHost::config { args } {
    ...
    set EAuth      [ list none auto chap_md5 pap papsender chapresponder papsenderorchapresponder ]
...
body PppoeHost::get_per_session { args } {
    set tag "body PppoeHost::get_per_session [info script]"
Deputs "----- TAG: $tag -----"
    foreach { key value } $args {
        set key [string tolower $key]
        switch -exact -- $key {
	...
	set root [ixNet getRoot]
	set view [CreatePPPoEPerSessionView]
	#set view {::ixNet::OBJ-/statistics/view:"PPPoX Per Session"}
    # set view  [ ixNet getF $root/statistics view -caption "Port Statistics" ]
Deputs "view:$view"
    set captionList             [ ixNet getA $view/page -columnCaptions ]
	# {Stat Name} {Port Name} {Session Name} {PPP State} {PPP Close Mode} 
	# {Session ID} {Host MAC Address} {Local IP Address} {Remote IP Address} 
	# {IPv6 Address} {IPv6 Prefix Length} {DNS Server List} {Establishment Time}
Deputs "caption list:$captionList"
	set port_name				[ lsearch -exact $captionList {Stat Name} ]
    set host_mac_address          [ lsearch -exact $captionList {Host MAC Address} ]
    set local_ip_address         [ lsearch -exact $captionList {Local IP Address} ]
	set remote_ip_address         [ lsearch -exact $captionList {Remote IP Address} ]
...
Deputs "chas:$chassis card:$card port$port"
    ...
    foreach row $stats {
        ...
        eval {set row} $row
Deputs "row:$row"
        set portname [ lindex $row $port_name ]
Deputs "portname:$portname"
        regexp -nocase {([0-9\.]+)/Card([0-9\.]+)/Port([0-9\.]+) .+} $portname match tchassis tcard tport
		
		if { $chassis !=$tchassis || $card != $tcard || $port !=$tport } {
			continue
		}
Deputs "$macaddr:[lindex $row $host_mac_address]"		
		if {[string tolower $macaddr ] != [string tolower [lindex $row $host_mac_address]] } {
		  continue
		}
        ...
        if { [ info exists ipaddr ] } {
		    ...
		    set value [ lindex $row $local_ip_address ]	
            Deputs "ipaddr $ipaddr: $value"			
		    uplevel 2 "set $ipaddr $value"
			Deputs "mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm"
			uplevel  "set $ipaddr $value"
	    }
